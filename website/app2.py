...
def upload_file():
    ...
    if session['concerns'] == []:
        return render_template('questionnaire2.html', message='Tell us a little more about yourself')
    else:
        update_csv(session['concerns'])
        return render_template('spa_results2.html', basket=session['basket'],product_categories=session['categories'])
    ...
    if request.method == 'POST':
        ...
        if concerns == []:
            ...
            if session['profile'] == 'True':
                ...
                return render_template('spa_results2.html', basket=session['basket'],
                                product_categories=session['categories'])

    elif session['concerns'] != []:
        concerns = session['concerns']
        return render_template('create_basket2.html',product_dictionary=session['product_categories'],
                            categories_show=session['categories'],concerns=session['concerns'],basket=session['basket'],
                            scripts=session['scripts'])
    else:
        return render_template('questionnaire2.html',message='Tell us a little about yourself so we make the best recommondations for you')
...
def individual_categories():
    ...
    if category == 'product':
        ...
        return render_template('products_page2.html',products=session['product_categories'][category],concerns=session['concerns'])
...
def add_to_basket():
    ...
    return render_template('update_basket2.html',basket=session['basket'],product_dictionary=session['product_categories'],
...
def upload():
    ...
    if session['concerns'] == []:
            # concerns = ['antiaging','skintone','sensitive']
        return render_template('questionnaire2.html',message='Start by telling us a little about yourself')
